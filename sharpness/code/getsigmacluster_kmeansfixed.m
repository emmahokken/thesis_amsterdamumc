function [ better_signed, worse_signed, brd_crds, brd_ind, AddRes ] = getsigmacluster( map_corr, data_corr, data_uncorr, pars, field_name, plotsavedir )
...
map = map_corr;
data = data_corr;
% if isfield(pars,'useUncorr') && pars.useUncorr==1
%   map = map_corr;
%   data = data_corr;
%   disp('Using uncorrected data for clustering')
% else
%   pars.useUncorr=0;
% end
...
% scale_I = pars.wI./max(abs(nonzeros(data.*(map<max_R3))));
...
%I_R3_sm = nonzeros(R3_val_sm); % Might form a problem in R1 maps -> they contain zeros in the data?
% MC: better masking method
...
kmeansfile=fullfile(pars.savedir,['kmeans_' pars.uID '.mat']);
if exist(kmeansfile,'file')
  ind_R3subcl=[]; k_subcl=[]; I_inds=[];
  disp(['loading ' kmeansfile])
  load(kmeansfile)
else
  ...
  disp(['saving ' kmeansfile])
  save(kmeansfile,'ind_R3subcl','k_subcl','I_inds')
end
k_tot = sum(k_subcl);
...
n_invalid = zeros(1,2);
aggregates = cell(1,2);
smeans=aggregates; sdiff=aggregates; mads=aggregates;
for p = 1:2
...
%     if pars.useUncorr
%       map=map_uncorr; % use uncorr if requested
%     end
    if p==1
        data = data_corr;
        ctag='corr'
    ...
    %map(~data)=mapInf; % saturate padding zeros
    map(~mask)=mapInf;
    ...
    R1_map = map>min_R1 & map<=max_R1;
    %R1_val = data.*R1_map;
    %I_R1 = nonzeros(R1_val);
    %DT_R1 = nonzeros(R1_map.*map);
    I_R1 = data(R1_map);
    DT_R1 = map(R1_map);
    
    R2_map = map>max_R1 & map<max_R2;
    %R2_val = data.*R2_map;
    %I_R2 = nonzeros(R2_val);
    %DT_R2 = nonzeros(R2_map.*map);
    I_R2 = data(R2_map);
    DT_R2 = map(R2_map);
    
    R3_map = map>max_R2 & map<max_R3;
    %R3_val = data.*R3_map;
    %I_R3 = nonzeros(R3_val);
    %DT_R3 = nonzeros(R3_map.*map);
    I_R3 = data(R3_map);
    DT_R3 = map(R3_map);
...
%         end
    ...
    f_fit = cell(1,k_tot);
    for i = 1:k_tot  
        % diff=h (height, contrast), smean=l (mean signal)
        [f_fit{i}, sigma(i), cb, sdiff(i), ~, sse,smean(i)] = errorfunctionFATNAV(prof_fit{i});
        l_R1 = length(nonzeros(ind_R1==i));
        err(i) = abs(cb(1)-sigma(i));
        cb_rel = 2*err(i)./sigma(i);
        sse_rel = sse/l_R1;
        
        % get median absolute difference
        xx=prof_fit{i}(:,1);
        yy=prof_fit{i}(:,2);
        mad(i)=median(abs(feval(f_fit{i},xx)-yy));
        
        % validity testing based on 3 criteria, build binary coding for
        % invalidity
        c1=abs(sdiff(i))>2*sse_rel; 
    ...
    % It might be useful to see whether the profiles
    % are allright. This plots the first 20 profiles and fit lines, which 
    % should be sufficient to see whether something is wrong. Only 
    % corrected profiles are plotted.
    colors=[0 0 0;0 0 1];
    lcolors=[1 0 0;0 0 0];
    %if p==1  
        ...
        % for parameter sweep, adjust name
        if isfield(pars,'run')
          name = [name '_' num2str(pars.run)];
        end
        
        filename = fullfile(plotsavedir,'Profiles',name);
...
AddRes.smeans = smeans; % l = mean = signal
AddRes.sdiffs = sdiffs;  % h = contrast
AddRes.mads = mads; % median absolute difference
end
