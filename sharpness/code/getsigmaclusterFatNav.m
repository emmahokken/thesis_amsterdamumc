function [ better_signed, worse_signed, brd_crds, brd_ind, AddRes ] = getsigmaclusterFatNav( map_rim, map_gt, data_rim, data_gt, pars, field_name, plotsavedir, FileID )
...
if isfile(kmeans_file) == 0
    ...
    save(kmeans_file,'ind_R3subcl','k_subcl','I_inds')
end
...
aggregates = cell(1,2);
for p = 1:2
    map = map_gt; % map_corr is used here, as it was used for clustering as well. map_corr and map_uncorr are expected to be nearly identical.
    if p==1
        data = data_rim;
