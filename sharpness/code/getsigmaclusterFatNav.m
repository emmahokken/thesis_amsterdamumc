function [ better_signed, worse_signed, brd_crds, brd_ind, AddRes ] = getsigmaclusterFatNav( map_recon, data_recon, data_gt, pars, field_name, plotsavedir )
...
% Load and scale data. Clustering is done on reconstructed data. Can be changed
...
for p = 1:2
    map = map_recon; % map_recon is used here, as it was used for clustering as well. map_corr and map_uncorr are expected to be nearly identical.
    if p==1
        data = data_recon;
        ctag='recon'
    else
...
%% Compare reconstructed and grount truth data
...
worse_signed = validboth.*sigma_diff_signed.*((sr-er)>(sg+eg));
unc = sqrt(abs(1./(sg.^2-sr.^2).*(sg.^2.*eg.^2+sr.^2.*er.^2)));

%% Store additional resultes 
...
AddRes.('sigmarecon') = sigma_comp{1}; % sigma for corrected data
AddRes.('errrecon') = err_comp{1}; % fit error for corrected data
AddRes.('sigmagt') = sigma_comp{2}; % sigma for uncorrected data
AddRes.('errgt') = err_comp{2}; % fit error for uncorrected data
